...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/05/12 13:51:36 by mde-jesu          #+#    #+#             */
/*   Updated: 2014/05/12 13:51:40 by mde-jesu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++i < e->sprites_nbr)
	{
		if (e->sprite_tab[i].tex_id == 13)
		{
			check = colision_check(e, e->sprite_tab[i].pos_x
									+ e->sprite_tab[i].dir_x * 0.8
									, e->sprite_tab[i].pos_y
									+ e->sprite_tab[i].dir_y * 0.8);
			if (check && (check == -1 || e->sprite_tab[i].plane_x == 0))
				apply_damages(e, check, &i);
			else if (!damage_player(e, &i))
			{
				e->sprite_tab[i].pos_x += e->sprite_tab[i].dir_x * 0.8;
				e->sprite_tab[i].pos_y += e->sprite_tab[i].dir_y * 0.8;
			}
		}
	}
}
